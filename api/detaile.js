...
지금 당장 필요한 것은, 지문을 읽고 주어진 선택지의 정보가 지문의 내용과 일치하지 않도록 수정해야 한다. 
되도록 선택지만 보아도 답이라는 것을 알 수 없도록 해야하며,
주어진 문장이 참조하고 있는 지문의 내용을 참조해서 대조했을 때 비로소 틀린 것을 판단할 수 있게끔 작성해야 한다. 
20단어 이내의 영어문장 하나로 써라.
다른 설명을 추가하는 것은 금지된다. 번호나 다른 마킹도 금지된다. 오직 네가 만든 문장만을 출력하라.`,
conste: `다음 영어지문의 내용과 일치하지 않는 것을 찾는 문제의 해설을 작성해야 한다. 답변에 대한 설명은 하지말고 아래 예시의 포맷에 맞추어 주어진 문제를 풀고 그에 대한 해설을 작성해 출력하라.

===포맷===
정답: circled number
"{정답과 관련있는 지문 내 문장 또는 어구}"({앞 어구에 대한 한국어 해석})라고 하였으므로, {정답번호} "{정답 선택지 문장}"({정답 선택지 문장의 한국어 해석})는 글의 내용과 일치하지 않는다. [오답] {선택지 번호와 문장(한국어해석)차례대로}
===예시===
정답: ⑤
"Do not underestimate yourself so that others can't, either"(다른 사람도 너를 과소평가할 수 없도록 너 자신을 과소평가하지마라)라고 하였으므로 ⑤ "You should not overestimate yourself."(당신은 스스로를 과대평가해서는 안된다.)는 글의 내용과 일치하지 않는다. [오답 해석] ① This is a test sentence. (이것은 시험 문장이다.) ② Another example follows here. (또 다른 예시가 여기에 이어진다.) ③ We are nearing the end. (우리는 끝에 다다르고 있다.) ④ Almost finished now. (이제 거의 끝났다.)  
===네가 해설을 만들어야할 문제===
{{p}}`,
};
