...
export default async function handler(req, res) {
  ...
  try {
    const result = await generateGisteProblem(passage);
    res.status(200).json(result);
  } catch (error) {
    console.error('giste API error:', error);
...
async function generateGisteProblem(passage) {
...
지금 당장 필요한 것은, 정답 선택지를 만드는 것이다. 정답 선택지는 지문의 요지를 담아내는 '~다' 체의 20단어 이내의 영어 문장이어야 한다. 설명 없이, 네가 만든 정답 선택지를 출력하라.
마킹이나 숫자는 입력하지말 것.
...
지금 당장 필요한 것은, 오답 선택지를 영어 문장으로 채우는 것이다. [중요!] 정답 선택지와 길이만 유사할 뿐 충분히 달라야 한다. (문장 구조나 단어를 흉내내는 것 절대 금지)
...
지금 당장 필요한 것은, 딱 하나 남은 오답 선택지를 영어 문장으로 채우는 것이다. [중요!] 다른 선택지와 충분히 달라야 한다. (문장 구조나 단어를 흉내내는 것 절대 금지) 
오답 선택지를 만들기 위해 영어지문에 사용된 어휘를 사용할 수는 있지만, 영어지문 내용과 부분적으로 일치하는 것이 오답이 되어서는 안된다. (복수정답 방지)
다른 설명 없이, 네가 만든 오답 문장을(번호 제외) 출력하라.
마킹이나 숫자는 입력하지말 것.
`,
...
===예시===
정답: ⑤
자신을 과대평가 또는 과소평가하지 말고 객관적으로 평가하라는 내용의 글이다. 따라서, 글의 요지는 ⑤가 가장 적절하다.
[정답 해석] ⑤ 인생에서 자신의 강점과 약점을 정확하게 평가하는 것이 중요하다.
[오답 해석] ① 디지털 정보 접근성에서 장애인 고려가 부족하다. ② 정보 접근성과 데이터 분석 효율성이 관련있다. ③ 웹 페이지 접근성 및 정보 획득의 중요성이 간과되고 있다. ④ 건축 설계의 핵심 고려 사항을 지켜야 한다.
=========
