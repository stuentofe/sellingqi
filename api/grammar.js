...
a: "주어와 동사의 수가 일치하도록 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
b: "부사 자리에 형용사가 쓰였으므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
c: "분사구문의 주어와 (능동 또는 수동 중에 맞는 것 택일)관계이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
d: "시제가 있는 수동태 동사구가 필요하므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
e: "이어지는 절이 완전하므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
f: "no rule",
g: "no rule",
h: "수식 받는 명사와 (능동 또는 수동 중에 맞는 것 택일)관계이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
i: "no rule",
j: "<it is ... that ~> 강조 구문이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
k: "no rule",
l: "문장의 주어 자리이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
m: "이어지는 절이 완전하고 의미상 장소를 나타내고 있으므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
n: "간접의문문 안에 알맞은 의문사가 사용되지 않았으므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
o: "앞선 명사와 동격을 이루는 명사절 접속사의 자리이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
p: "사역동사 뒤에는 목적격 보어 자리에 to부정사가 올 수 없으므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
q: "few는 복수 취급하므로 동사의 수일치를 시키기 위해 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
r: "대동사 자리에는 앞 문장에서 반복된 동사를 대신하는 형태를 써야 하므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
s: "no rule",
t: "재귀대명사가 필요한 자리에 인칭대명사가 쓰였으므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
u: "no rule",
v: "병렬 구조에서는 나열된 항목들의 형태를 일치시켜야 하므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
w: "목적격보어 자리로 부사가 아닌 형용사를 써야 하므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다.",
x: "(기존 형태가 틀렸고 옳은 형태가 와야 하는 이유 언급)이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다."
};
