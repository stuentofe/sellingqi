...
a: `"(수일치관련언급)이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다"라는 형식의 한국어로 설명한다.`,
b: `"동사를 수식하는 부사이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다"라는 형식의 한국어로 설명한다.`,
c: `""라는 형식의 한국어로 설명한다.`,
d: `"문장의 본동사 역할을 하는 수동태이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다"라는 형식의 한국어로 설명한다.`,
e: `"이어지는 절이 완전하므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다`,
f: `"<so 형용사/부사 that S V>구문이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다"라는 형식의 한국어로 설명한다.`,
g: `"<it-to부정사 가주어-진주어> 구문이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다"라는 형식의 한국어로 설명한다.`,
h: `"수식받는 명사와 (능동 또는 수동) 관계 이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다"라는 형식의 한국어로 설명한다.`,
i: '15자 이내의 한국어로 설명한다.',
j: '15자 이내의 한국어로 설명한다.',
k: '15자 이내의 한국어로 설명한다.',
l: '15자 이내의 한국어로 설명한다.',
...
v: '15자 이내의 한국어로 설명한다.',
w: '15자 이내의 한국어로 설명한다.',
x: `"(정답이 되는 이유)이므로 (기존 형태)를 (옳은 형태)로 고쳐야 한다"라는 형식의 한국어로 설명한다.`
};
