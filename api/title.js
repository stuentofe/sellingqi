...
export default async function handler(req, res) {
  ...
  try {
    const result = await generateTitleProblem(passage);
...
async function generateTitleProblem(passage) {
  ...
  const question = `다음 글의 제목으로 가장 적절한 것은?\n${fullPassage}\n\n${sorted.map(e => e.numbered).join('')}`;
...
const inlinePrompts = {
 ...
 // --- TITLE 유형 ---
  ...
  constc: `영어 지문을 읽고 글의 제목을 파악해서 고르는 객관식 문제를 만들려고 한다. 지금 현재 준비된 것은 다음과 같다.
...
여러 다양한 제목의 스타일들 중에서 하나를 골라 10단어 이내로 만들어라. 
마킹이나 숫자는 입력하지말 것.

`,


  constw: `영어 지문을 읽고 글의 제목을 파악해서 고르는 이지선다형 객관식 문제를 만들려고 한다. 지금 현재 준비된 것은 다음과 같다.
...
지금 당장 필요한 것은, 오답 선택지를 채우는 것이다. [중요!] 정답 선택지와 길이만 유사할 뿐 스타일이 충분히 달라야 한다. (똑같은 단어로 시작하는 것은 금지)
...
`,
  ...
  constx: `영어 지문을 읽고 글의 제목를 파악해서 고르는 삼지선다형 객관식 문제를 만들려고 한다. 지금 현재 준비된 것은 다음과 같다.
...
지금 당장 필요한 것은, 딱 하나 남은 오답 선택지를 채우는 것이다. [중요!] 다른 선택지와 스타일이 충분히 달라야 한다. (똑같은 단어로 시작하는 것은 금지)
오답 선택지를 만들기 위해 영어지문에 사용된 어휘를 사용할 수는 있지만, 영어지문 내용과 부분적으로 일치하는 것이 오답이 되어서는 안된다. (복수정답 방지)
다른 설명 없이, 네가 만든 오답 제목을(번호 제외) 출력하라. (네가 만든 것만 출력하고 나머지는 적지 말것)
마킹이나 숫자는 입력하지말 것.
`,

 consty: `영어 지문을 읽고 글의 제목를 파악해서 고르는 사지선다형 객관식 문제를 만들려고 한다. 지금 현재 준비된 것은 다음과 같다.
...
`,
  ...
  constz: `영어 지문을 읽고 글의 제목을 파악해서 고르는 오지선다형 객관식 문제를 만들려고 한다. 지금 현재 준비된 것은 다음과 같다.
...
`,
  ...
  conste: `다음 영어지문의 제목을 파악하는 문제의 해설을 작성해야 한다. 다른 설명은 하지말고 아래 예시의 포맷에 맞추어 주어진 문제를 풀고 그에 대한 해설을 작성해 출력하라.
