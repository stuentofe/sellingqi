...
const inlinePrompts = {
  ...
  forcedMarking: `Never respond in conversational form. Answer carefully after reading all the conditions below:\nPick one and only one word that has an antonym and that is not an element of a compound word. Output the whole sentence again with the selected word wrapped with angle brackets <>.\nSentence: {{s}}`,
  ...
  corruptExplain: `{{s}}이 틀렸기 때문에 {{original}}으로 고쳐야 하는 이유를 설명하는 해설을 한국어로 작성하라. (다음 예시를 참고하여 총50자 이내로 작성할 것. <예시> ~~인 글이다. 따라서, ~를 ~와 같은 낱말로 바꿔야 한다.`
};
